...
import { renderTimeBar, addSvg, fitSvgToContainer, miniMap } from 'khulnasoft-vis-js'
...
export const SimplePanel: React.FC<Props> = ({ id, data, width, height }) => {
  ...
  const onDetailBtnClick = useCallback(async (_ids: string[]) => {
    setErrMsg('')
    const khulnaSoftName = await getDataSourceSrv()
      .getList()
      .find((dataSource: DataSourceInstanceSettings) => {
        return dataSource.type === 'khulnasoft-khulnasoft-datasource'
      })?.name
    const khulnaSoft = await getDataSourceSrv().get(khulnaSoftName)
    if (!khulnaSoft) {
      return
    }
    try {
      ...
      // @ts-ignore
      const result = await khulnaSoft.getFlameRelatedData(_ids)
      if (!result) {
  ...
  return (
    <div ref={panelRef} className={`khulnasoft-panel ${isDark ? 'semi-always-dark' : 'semi-always-light'}`}>
