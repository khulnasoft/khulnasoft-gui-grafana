...
import './AskGPT.css'
import { findLastVisibleTextNode, getKhulnaSoftDatasource } from 'utils/tools'
...
const system_content = `
  ...
  数据的格式为一个JSON数组，其中khulnasoft_span_id表示当前span的id，khulnasoft_parent_span_id表示其父亲span的id，整个调用链通过这两个字段链接起来，如果一个span的khulnasoft_parent_span_id为空字符串，那么它就是初始的span。
...
export const AskGPT: React.FC<Props> = ({ data }) => {
  ...
  const onStartRequestClick = async () => {
    const khulnaSoft = await getKhulnaSoftDatasource()
    if (!khulnaSoft) {
      return
    }
    ...
    try {
      ...
      // @ts-ignore
      await khulnaSoft.askGPTRequest(engine, postData, receiveFn)
    } catch (error: any) {
      setDrawerData({
        ...drawerData,
        inRequest: false,
  ...
  const getAiEngines = async () => {
    try {
      const khulnaSoft = await getKhulnaSoftDatasource()
      if (!khulnaSoft) {
        throw new Error('Please check if KhulnaSoft datasource is enabled')
      }
      setAiEngines([])
      // @ts-ignore
      const result = await khulnaSoft.getAIConfigs()
      ...
      setDrawerData({
        inRequest: false,
        answer: `<a style="margin: 10px 0; text-decoration: underline; color: #6e9fff; display: block;" href="https://khulnasoft.io/docs/zh/best-practice/production-deployment/#%E4%BD%BF%E7%94%A8ai%E6%A8%A1%E5%9E%8B" target="_blank">Engine帮助文档</a>`,
        answerIsEnd: true
      })
      setTimeout(() => {
        setErrorMsg('')
  ...
  return (
    <div>
      <Button
        ...
        tooltip="Ask GPT, support by KhulnaSoft"
        onClick={() => {
          setVisible(true)
        }}
      >
