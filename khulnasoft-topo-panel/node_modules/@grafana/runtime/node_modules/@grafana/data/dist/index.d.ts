...
import React$1, { ComponentType, ReactNode, ComponentClass, PropsWithChildren, ReactElement } from 'react';
...
interface ScopedVars extends Record<string, ScopedVar> {
}
 ...
 * @internal
 */
interface AbstractQuery extends DataQuery {
 ...
 * @internal
 */
interface DataSourceWithQueryImportSupport<TQuery extends DataQuery> {
    importFromAbstractQueries(labelBasedQuery: AbstractQuery[]): Promise<TQuery[]>;
}
/**
 * @internal
 */
interface DataSourceWithQueryExportSupport<TQuery extends DataQuery> {
    exportToAbstractQueries(query: TQuery[]): Promise<AbstractQuery[]>;
}
/**
 * @internal
 */
declare const hasQueryImportSupport: <TQuery extends DataQuery>(datasource: unknown) => datasource is DataSourceWithQueryImportSupport<TQuery>;
/**
 * @internal
 */
declare const hasQueryExportSupport: <TQuery extends DataQuery>(datasource: unknown) => datasource is DataSourceWithQueryExportSupport<TQuery>;
...
declare const isDateTime: (value: any) => value is DateTime;
...
declare type DisplayProcessor = (value: any, decimals?: DecimalCount) => DisplayValue;
...
declare const preferredVisualizationTypes: readonly ["graph", "table", "logs", "trace", "nodeGraph", "flamegraph"];
...
interface BaseVariableModel extends VariableModel {
    name: string;
    label?: string;
    id: string;
...
declare function describeTextRange(expr: any): any;
/**
 ...
 * @alpha
 */
interface MapLayerOptions<TConfig = any> {
    type: string;
    name: string;
    config?: TConfig;
...
declare const MISSING_VALUE: any;
 ...
 */
declare function makeClassES5Compatible<T>(ES6Class: T): T;
 ...
 */
declare function guessFieldTypeFromNameAndValue(name: string, v: any): FieldType;
/**
 * Check the field type to see what the contents are
 */
declare function getFieldTypeFromValue(v: any): FieldType;
/**
 * Given a value this will guess the best column type
 *
 * NOTE: this is will try to see if string values can be mapped to other types (like number)
 */
declare function guessFieldTypeFromValue(v: any): FieldType;
/**
...
declare const isTableData: (data: any) => data is DataFrame;
declare const isDataFrame: (data: any) => data is DataFrame;
/**
...
/** @public */
declare type ValueConverter<T = any> = (val: any) => T;
/**
 ...
 */
declare function transformDataFrame(options: DataTransformerConfig[], data: DataFrame[]): Observable<DataFrame[]>;
...
declare const escapeStringForRegex: (value: string) => string;
declare const unEscapeStringFromRegex: (value: string) => string;
