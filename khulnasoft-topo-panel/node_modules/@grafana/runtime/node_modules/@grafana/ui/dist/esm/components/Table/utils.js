...
import { getFieldDisplayName, FieldType, formattedValueToString, fieldReducers, ArrayVector, getDisplayProcessor, reduceField } from '@grafana/data';
...
function getColumns(data, availableWidth, columnMinWidth, expandedIndexes, setExpandedIndexes, expander, footerValues) {
  var _a;
  const columns = expander ? [
    {
      Header: () => null,
      id: "expander",
      Cell: ({ row }) => {
        return /* @__PURE__ */ React__default.createElement(RowExpander, {
          row,
          expandedIndexes,
          setExpandedIndexes
        });
  ...
  for (const [fieldIndex, field] of data.fields.entries()) {
    ...
    columns.push({
      ...
      minWidth: (_a = fieldTableOptions.minWidth) != null ? _a : columnMinWidth,
...
function getFooterItems(filterFields, values, options, theme2) {
  return filterFields.map((data, i) => {
    ...
    if (options.fields && options.fields.length > 0) {
      const f = options.fields.find((f2) => f2 === data.field.name);
      if (f) {
...
export { EXPANDER_WIDTH, calculateUniqueFieldValues, createFooterCalculationValues, filterByValue, getCellComponent, getColumns, getFilteredOptions, getFooterItems, getTextAlign, rowToFieldValue, sortCaseInsensitive, sortNumber, sortOptions, valuesToOptions };
